...
public class AccountDaoImpl extends BaseDao<Account>  implements AccountDao {
    ...
    public List<Account> findAll() {
        String sql= "select * from accountjdbc";
       return super.query(sql, accountMapper);
    }

    @Override
    public Account findById(Long id) {
        String sql= "select * from accountjdbc where id=?";
      return super.query(sql, accountMapper, id).stream().findAny().orElse(null);
    }

    @Override
    public void insert(Account account) {
        String sql="insert into accountjdbc(username, password, firstname, lastname, email, status, create_date) \n" +
                "values (?,?,?,?,?,?,?) ";
